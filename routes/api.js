...
router.post('/empath-analysis', function (req, res) {
  ...
  var formData = {
    ...
    wav:fs.createReadStream( './uploads/wav_files/'+ path +'.wav' )  //fs.createReadStream("./resources/0wuqx-scsny.wav") //should be .wav format, shouldn't exceed 5s, 1.9MB  and frequency should be 11025 Hz
  };  
  ...
  if (err) {
    ...
    res.json(apiResponse.success(respBody))
  }     
  });
});
...
router.post('/convert-ffmpeg-11025',upload.single('audio'), function(req,res,next){
  let track = req.file;
  ffmpeg('./uploads/'+ track.originalname)
  .audioChannels(1)
  .withAudioFrequency(11025)  
  .toFormat('wav')
  .on('error', (err) => {	  
      res.json(apiResponse.customError(err.message));	     
  }).on('progress', (progress) => {
